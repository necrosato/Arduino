...
void testdrawtriangle(void) {
  for (int16_t i=0; i<_min(display.width(),display.height())/2; i+=5) { //_min must be used when including ESP8266WiFi.h
...
void testfilltriangle(void) {
  uint8_t color = WHITE;
  for (int16_t i=_min(display.width(),display.height())/2; i>0; i-=5) { //_min must be used when including ESP8266WiFi.h
